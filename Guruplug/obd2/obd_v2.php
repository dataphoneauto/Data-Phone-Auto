   ...
   //$value= ">0100\nSEARCHING...\n41 00 01 00 00 01\n";
    ...
    ###############################################################################################################
        ...
        while(true)
        {
            ...
            {
                echo "OUI\n";
                $filelogBDD = open_file_cmd("/var/www/obd2/libraryPID/codeBBD.txt");
                while (!feof($filelogBDD)) {                           //on parcourt toutes les lignes du fichier de commandes
                    $logBDD = fgets($filelogBDD, 200);                           //lecture de la ligne
                    if ($logBDD[0] != "#"){
                        echo"$logBDD";   
                        write_data($filelogetat,$logBDD);
                    }
...
//    $postvars= "data=11-EEE-75|4|$senddata";
   	$identifiant[0] = htmlspecialchars($identifiant[0]);
...
function recuptimesynchro()
{
    ...
    $time = fgets($file);
    fclose($file);
    $temps = explode(' ',$time);            //garder la partie avant le ' '
    if( $temps[1] == "min")
